...
from app.services.file_export import export_product_files
...
async def trigger_product_export():
    ...
    try:
        export_files = export_product_files(state)
...
async def download_product_export(format: str):
    ...
    file_path_str = state.export_files.get(format) if state.export_files else None
    ...
    if not file_path or not file_path.exists():
        ...
        try:
            ...
            state.export_files = {fmt: str(path) for fmt, path in export_files.items()}
            save_product_state(state)
            file_path_str = export_files.get(format)
            file_path = Path(file_path_str) if file_path_str else None
        except Exception as e:
...
class DraftMultiviewRequest(BaseModel):
    prompt: str = Field(..., min_length=3, max_length=2000)
    image_url: str = Field(...)
...
async def generate_draft(request: DraftRequest):
    ...
    img = await asyncio.to_thread(
        gemini_image_service._generate_single_image,
        prompt=request.prompt,
        reference_images=None,
        model=gemini_image_service.pro_model,
        angle_index=0
    )
    return {"image_url": img}

@router.post("/draft-edit")
async def edit_draft(request: DraftEditRequest):
    from app.integrations.gemini import gemini_image_service
    img = await asyncio.to_thread(
        gemini_image_service._generate_single_image,
        prompt=request.prompt,
        reference_images=[request.image_url],
        model=gemini_image_service.flash_model,
        angle_index=0,
        base_description=""
    )
    return {"image_url": img}

@router.post("/draft-multiview")
async def draft_multiview(request: DraftMultiviewRequest):
    from app.integrations.gemini import gemini_image_service
    
    async def _gen(index):
        return await asyncio.to_thread(
            gemini_image_service._generate_single_image,
            prompt=request.prompt,
            reference_images=[request.image_url],
            # Use the edit model to preserve the base image's appearance exactly
            model=gemini_image_service.flash_model,
            angle_index=index,
            base_description="The exact same product in the reference image"
        )
        
    # We already have the front view (index 0). Generate angles 1, 2, 3 in parallel
    results = await asyncio.gather(_gen(1), _gen(2), _gen(3))
    
    # Return all 4 views (front + 3 new)
    return {"images": [request.image_url] + list(results)}
